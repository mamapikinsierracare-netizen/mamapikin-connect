      ...
      <div className="min-h-screen bg-gray-50 py-8">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="flex justify-center mb-4">
            <div className="relative h-28 w-28 rounded-full bg-white p-2 shadow-md overflow-hidden">
              <Image src="/logo.png" alt="MamaPikin Connect Logo" fill className="object-contain" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {features.map((feature) => (
              <Link key={feature.path} href={feature.path} className={`${feature.color} rounded-lg p-4 transition-all transform hover:scale-105 shadow-md`}>
                <div className="text-3xl mb-2">{feature.icon}</div>
                <div className="flex justify-between items-center">
                  <h3 className="font-bold text-gray-800">{feature.name}</h3>
                  {feature.badge && <span className="bg-red-500 text-white text-xs px-2 py-1 rounded-full">{feature.badge}</span>}
                </div>
